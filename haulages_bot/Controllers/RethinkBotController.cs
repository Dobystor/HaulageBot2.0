...
namespace haulages_bot.Controllers
{
    ...
    public class RethinkBotController : ControllerBase
    {
        ...
        /// <summary>Diagnóstico: obtener info de la tabla HaulageProcess (incluye primary_key)</summary>
        [HttpGet("{serverId}/tableinfo")]
        public async Task<IActionResult> TableInfo(int serverId)
        {
            var config = await _dbContext.RethinkBotConfigs
                .FirstOrDefaultAsync(c => c.ServerConfigId == serverId);
            ...
            try
            {
                // ReQL: r.db('SmartFlow').table('HaulageProcess').info()  (INFO = 79)
                var reql = "[1,[79,[[15,[[14,[\"SmartFlow\"]],\"HaulageProcess\"]]]],{\"binary_format\":\"raw\",\"time_format\":\"raw\",\"profile\":false}]";
                ...
                var tmpFile = $"/tmp/reql_info_{serverId}.bin";
                var queryBytes = System.Text.Encoding.UTF8.GetBytes(reql);
                var payload = new byte[8 + queryBytes.Length];
                System.BitConverter.GetBytes(1L).CopyTo(payload, 0);
                queryBytes.CopyTo(payload, 8);
                ...
                using (var p = System.Diagnostics.Process.Start(psi2))
                {
                    if (p != null)
                    {
                        response = await p.StandardOutput.ReadToEndAsync();
                        await p.WaitForExitAsync();
                    }
                    try { System.IO.File.Delete(tmpFile); } catch { }
                }
            ...
            {
                return StatusCode(500, new { message = "Error al obtener info", detail = ex.Message });
            }
        }
    }
}
