        ...
        public async Task<IActionResult> GetHaulages([FromQuery] int serverId, [FromQuery] int limit = 100)
        {
            var query = from h in _context.Haulages.Where(h => h.ServerConfigId == serverId)
                        ...
                        select new
                        {
                            ...
                            VehicleEconomicNumber = v != null ? v.EconomicNumber : (h.VehicleEconomicNumber ?? ""),
                            EmployeeId = h.EmployeeId,
                            EmployeeFullName = e != null ? e.FullName : (h.EmployeeFullName ?? ""),
                            PathId = h.PathId,
                            RouteDescription = r != null ? r.description : (h.RouteDescription ?? ""),
                            Weight = h.Weight,
