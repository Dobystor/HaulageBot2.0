$(document).ready(function () {
    ...
    // Renderizar lista de rutas con badge de tipo de material
    function renderRoutesList(routes, selectedList) {
        ...
        // Mapa de tipo de material a badge visual
        const materialBadge = {
            1: '<span style="font-size:9px;padding:1px 5px;border-radius:4px;background:rgba(255,215,0,0.15);color:#ffd700;border:1px solid rgba(255,215,0,0.3);font-weight:700;margin-left:4px;vertical-align:middle;">MINERAL</span>',
            2: '<span style="font-size:9px;padding:1px 5px;border-radius:4px;background:rgba(255,100,50,0.15);color:#ff6432;border:1px solid rgba(255,100,50,0.3);font-weight:700;margin-left:4px;vertical-align:middle;">ESTÉRIL</span>',
            3: '<span style="font-size:9px;padding:1px 5px;border-radius:4px;background:rgba(0,200,180,0.15);color:#00c8b4;border:1px solid rgba(0,200,180,0.3);font-weight:700;margin-left:4px;vertical-align:middle;">AMBOS</span>'
        };

        routes.forEach(route => {
            const routeId = route.haulagePathId;
            const routeName = route.description;
            const matType = route.selectedMaterialType || 1;
            const badge = materialBadge[matType] || materialBadge[1];
            const isChecked = selectedList.includes(routeId) ? 'checked' : '';

            const itemHtml = `
                <div class="checkbox-item" data-name="${routeName.toLowerCase()}">
    ...
    setupSearchFilter('searchRoutes', 'routesList');
